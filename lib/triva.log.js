...
export const log = {
  ...
  async push(req) {
    ...
    const lastID = (await cache.get("last_logID")) ?? 0;
    const id = lastID + 1;
    await cache.set("last_logID", id);
    ...
    await cache.push("logs", logEntry);
